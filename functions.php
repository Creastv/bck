...
function change_calendar_item_date($item, $post) {
	...
	// $ini = date("d/m/Y", strtotime(get_field('data', $post->ID)));
	// $ini =get_field('data', $post->ID);
    // $fi = date("d/m/Y", strtotime(get_field('data', $post->ID) ));

	// $date = DateTime::createFromFormat('d/m/Y', $ini);
...
function wpb_custom_cron_func() {
	$args = array(
	  ...
	  if ($events) : 
		...
		$rowDate = get_field('data', $event->ID);
        $date = date("d/m/Y", strtotime($rowDate));
		  ...
		  update_field('formularz', 'test' , $event->ID);
		  if($date <= $now){
			  if($hasRecurring){
				  foreach( $eventsRecurring as $ev){
					  if(date("d/m/Y", strtotime($ev['data'])) >= $now) {
						  $dob_str = $ev['data'];
						  $date = DateTime::createFromFormat('d/m/Y', $dob_str);
						  $date = $date->format('Ymd');
